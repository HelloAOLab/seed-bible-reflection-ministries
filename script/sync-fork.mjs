...
program
  ...
  .option(
    "--label <label>",
    "The label to apply to the pull request.",
    "fork:sync"
  )
  .option(
    "--merge-method <mergeMethod>",
    "The method to use for merging the pull request.",
    "merge"
  )
  .option(
    "--merge-attempt-count <mergeAttemptCount>",
    "The number of times to attempt to merge the pull request.",
    10
  )
  .action(async (current, upstream, options) => {
    const { auth, secret, merge, label, mergeMethod, mergeAttemptCount } =
      options;
    ...
    try {
      ...
      if (merge) {
        ...
        while (counter < mergeAttemptCount) {
          ...
          if (mergablePr.data.mergeable) {
            ...
            if (merge.data.merged) {
              console.log(
                `Pull request #${pr.data.number} merged successfully.\n${merge.data.sha.slice(0, 7)}: ${merge.data.message}`
              );
              await cleanup();
            }
            break;
            ...
            console.log(
              `Pull request #${pr.data.number} is not able to be merged. Skipping merge.`
            );
            break;
          }
        ...
        if (!merged) {
          console.error(
            `Failed to merge pull request #${pr.data.number} after ${counter} attempts.`
          );
          process.exit(1);
        }
      ...
      if (err instanceof RequestError) {
        ...
        if (/no commits between/gi.test(message)) {
          ...
          console.log(
            "A pull request already exists to sync these changes. Skipping because this PR must be manually merged."
          );
          return;
        } else {
          throw err;
