...
if(thisBot.masks.isSelected)
{
    ...
    await Promise.all(reversedVerses.map((verse, verseIndex) => {
        return verse.Hide({verseIndex, dimension, delayBetweenAnimations, duration})
    }))
    thisBot.vars.verses.forEach((verse) => {ObjectPooler.ReleaseObject({obj: verse, tag: verse.tags.poolTag});})
