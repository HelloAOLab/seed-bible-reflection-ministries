...
const mapBookStructure = thisBot.vars.mapBooksStructure.find((structure) => {return structure.dateLabel.id === label.id})
const {author, relativeDateRange, numberOfChapters} = StacksManager.tags.booksStaticInfo[mapBookStructure.mapBookData.elementInfo.commonName];
...
const mapData = mapBookStructure.mapBookData.parentDataIds && mapBookStructure.mapBookData.parentDataIds.mapId ? thisBot.GetMapDataById({mapId: mapBookStructure.mapBookData.parentDataIds.mapId}) : null;
...
if(mapBookStructure && (!mapData || !mapData.currentPlaylistShownId))
{
    ...
    if(structureCurrentlyShowingInfoCard)
    {
        ...
        else
        {
            ...
            const dateLabelPosition = getBotPosition(mapBookStructure.dateLabel, dimension);
            ...
            applyMod(structureCurrentlyShowingInfoCard.infoCardTransformer, transformerMod);
            applyMod(structureCurrentlyShowingInfoCard.infoCardContent, contentMod);
            applyMod(structureCurrentlyShowingInfoCard.infoCardBackground, backgroundMod);
            mapBookStructure.infoCardTransformer = structureCurrentlyShowingInfoCard.infoCardTransformer;
            mapBookStructure.infoCardContent = structureCurrentlyShowingInfoCard.infoCardContent;
            mapBookStructure.infoCardBackground = structureCurrentlyShowingInfoCard.infoCardBackground;
        }
    ...
    else
    {
        ...
        const backgroundScales = new Vector3(contentScales.x + (backgroundPadding*2), contentScales.y + (backgroundPadding*2), 0.1)
        const contentPosition = new Vector3(0, contentMarginBottom + (contentScales.y/2), 0);
        const infoCardTransformer = ObjectPooler.GetObjectFromPool({tag: ObjectPoolTags.MapBookInfoCardTransformer});
        const infoCardContent = ObjectPooler.GetObjectFromPool({tag: ObjectPoolTags.MapBookInfoCardContent});
        const infoCardBackground = ObjectPooler.GetObjectFromPool({tag: ObjectPoolTags.MapBookInfoCardBackground});
        const dateLabelPosition = getBotPosition(mapBookStructure.dateLabel, dimension);
        const dateLabelScales = GetBotScales(mapBookStructure.dateLabel);
        const transformerMod = {
            [dimension]: true,
